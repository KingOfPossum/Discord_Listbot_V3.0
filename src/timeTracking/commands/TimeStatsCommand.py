...
class TimeStatsCommand(Command):
    ...
    async def execute(self, ctx):
        ...
        entries = sorted(self.time_database.get_all_time_entries(user_id=ctx.author.id),key=lambda t: t.time_spent,reverse=True)
        ...
        for user in self.time_database.get_users():
            ...
            async def user_callback(interaction: discord.Interaction, current_user=user_entry.user_name):
                ...
                current_page = 1
                entries = sorted(self.time_database.get_all_time_entries(user_id=selected_user.user_id),key=lambda t: t.time_spent,reverse=True)
                page_amount = (len(entries) + self.ENTRIES_PER_PAGE - 1) // self.ENTRIES_PER_PAGE
