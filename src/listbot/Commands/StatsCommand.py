...
class StatsCommand(Command):
    ...
    async def execute(self, ctx):
        ...
        game_entries = self.list_database.get_all_game_entries()

        highest_rated_games = [(entry.name,entry.rating) for entry in sorted(game_entries, key=lambda x: x.rating,reverse=True)[:3]]
