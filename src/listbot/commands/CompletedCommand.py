...
class CompletedCommand(Command):
    """
    A command that allows users to change the completion status of a game in the database.
    """

    @staticmethod
    async def change_completed_status(game_name: str,ctx: discord.Interaction = None,interaction: discord.Interaction = None) -> GameEntry | None:
        """
        ...
        """
        game = await BotUtils.game_exists(game_name,ctx=ctx,interaction=interaction)
        if game is None:
            return None
        ...
        DatabaseCollection.list_database.put_game(new_game_entry)
    ...
    async def execute(self, ctx):
        ...
        new_game_entry = await self.change_completed_status(game_name=game_name,ctx=ctx)
